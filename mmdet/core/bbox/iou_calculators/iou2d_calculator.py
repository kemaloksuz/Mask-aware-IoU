    ...
    if is_aligned:
        ...
        if mode in ['giou', 'diou']:
            ...
            enclosed_rb = torch.max(bboxes1[..., :, None, 2:],
                                    bboxes2[..., None, :, 2:])
        if mode == 'diou':
            center_x1 = ((bboxes1[..., 2] + bboxes1[..., 0]) / 2).unsqueeze(1).expand_as(overlap)
            center_y1 = ((bboxes1[..., 3] + bboxes1[..., 1]) / 2).unsqueeze(1).expand_as(overlap)
            center_x2 = ((bboxes2[..., 2] + bboxes2[..., 0]) / 2).expand_as(overlap)
            center_y2 = ((bboxes2[..., 3] + bboxes2[..., 1]) / 2).expand_as(overlap)
